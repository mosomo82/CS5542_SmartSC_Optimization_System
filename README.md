...
[![HyperLogistics CI](https://github.com/mosomo82/CS5542_SmartSC_Optimization_System/actions/workflows/ci.yml/badge.svg)](https://github.com/mosomo82/CS5542_SmartSC_Optimization_System/actions/workflows/ci.yml)
...
1. **US Accidents (2016 - 2023)** — _Route-Level Disruption Risk_
...
2. **NOAA Global Surface Summary of the Day (GSOD)** — _Disruption Trigger Signals_
...
3. **National Bridge Inventory (US DOT)** — _Hard Safety Constraint_
...
4. **DataCo Smart Supply Chain (Kaggle)** — _Historical Pattern Training_
...
5. **Logistics Operations Database (Kaggle)** — _RAG Knowledge Base_
...
- **US Accidents** → _"Where are the historical risk zones along freight corridors?"_ — Enables proactive avoidance of accident blackspots.
- **NOAA Weather** → _"What real-time environmental conditions could disrupt transit?"_ — Provides the disruption trigger signals (icing, flooding, low visibility) that initiate the rerouting workflow.
- **Bridge Inventory** → _"Can the vehicle physically traverse this alternate route?"_ — Enforces hard physical constraints (clearance, load limits) that no AI reasoning can override.

Together, these datasets answer the three fundamental middle-mile questions: _risk, trigger_, and _feasibility_.
...
- 📄 **[ARCHITECTURE.md](./ARCHITECTURE.md)** — 4-layer system design, Medallion data architecture, and CPP/ReMindRAG details.
- 📊 **[EVALUATION.md](./EVALUATION.md)** — 50-scenario golden dataset results, 5-dim rubric scoring, and CI/CD quality gates.
- 👥 **[CONTRIBUTIONS.md](./CONTRIBUTIONS.md)** — Detailed log of Lab 9 work for each team member.

### **Changelog**

- **Unified Dashboard:** Merged Lab 6 analytics (Fleet, Fuel, Safety) with Lab 8 CPP dispatcher interface.
- **Safety Hard Gate:** Implemented deterministic Spatial SQL validation (Step 3A) for 100% truck height/weight compliance.
- **Explainable RAG:** Wired ReMindRAG traversal into the retrieval layer with evidence injection to eliminate LLM hallucinations.
- **Unified CI:** Created a master GitHub Action suite running pipeline smoke tests, CPP unit tests, and system evals.

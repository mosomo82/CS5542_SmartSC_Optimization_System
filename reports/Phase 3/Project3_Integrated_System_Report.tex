...
\begin{itemize}[nosep]
  ...
  \item \textbf{Domain-adapted language models} (QLoRA-tuned Phi-2 for logistics reasoning)
\end{itemize}
Dispatchers prompt the agent with natural language commands (e.g., ``Route the 80,000\,lbs shipment via I-80 despite the blizzard'') and receive an instant, 4-step Chain-of-Thought (CoT) decision with structural safety guarantees.
...
\hspace{1em}Step 3A: Spatial SQL Gate (deterministic, cannot override) \\
\hspace{1em}Step 3B: Domain-Adapted LLM (Phi-2 + QLoRA, 4-step CoT) \\
\hspace{3em}$\downarrow$ \\[4pt]
LAYER 4 --- APPLICATION \\
...
\begin{itemize}[nosep]
  \item \textbf{Environment Configuration:} Unified \texttt{requirements.txt} locking all critical libraries (\texttt{torch}, \texttt{snowflake-connector-python}, \texttt{transformers}), avoiding Windows/Linux binding conflicts in CI/CD.
  \item \textbf{Dependency Management:} GitHub Actions CI running on Python 3.10 with containerized testing modules.
  \item \textbf{LLM Tools for Debugging:} Used LLM-assisted code generation to produce deterministic unit tests (\texttt{test\_repro\_variance.py}) that track identically hashed chunk outputs, bounding RAG variance to near zero. LLM tools also debugged Snowflake connector authentication issues and resolved \texttt{bitsandbytes} CUDA compatibility errors during QLoRA setup.
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Real Model Evaluation: 63-Query Test (Colab T4 GPU, QLoRA-adapted Phi-2)}
\label{tab:strategy}
\begin{tabular}{lc}
\toprule
...
\textbf{Key Finding:} The model exhibits strong VETO bias---it correctly identifies nearly all safety-critical VETO scenarios (91\% recall) but tends to over-veto APPROVE cases. This is a \textit{safety-conservative} behavior that is desirable for logistics compliance: the system will never approve a physically dangerous route, though dispatchers may need to override false vetoes on safe routes.
